...
def parse_args():
    ...
    parser.add_argument('--metric', type=str, default='mir', choices=['r1', 'r5', 'medr', 'meanr', 'mir', 'sum_recall', 'negRank'],
    ...
    parser.add_argument('--save_negative', type=int, default=0, choices=[0,1],
    ...
    parser.add_argument('--nVnormalize_pooling', type=int, default=0, choices=[0,1])
...
def main():
    ...
    model_path = os.path.join(rootpath, trainCollection, 'sea_train', valCollection, val_set, opt.config_name, opt.model_prefix)
    if util.checkToSkip(os.path.join(model_path, 'model_best.pth.tar'), opt.overwrite or opt.evaluate):
        sys.exit(0)
    ...
    if opt.resume:
        ...
        rnn_vocab_file = os.path.join(rootpath, resume_trainCollection, 'TextData', 'vocab', 'gru_%d.pkl'%(config.threshold))
    else:
    ...
    if hasattr(config, 'model') and config.model in ['multispace_visnetvlad_bow_w2v']:
        train_loader = data.frame_pair_provider({'vis_feat': vis_feat_files['train'], 'capfile': cap_file_paths['train'], 'pin_memory': True,
                                                  'batch_size': opt.batch_size, 'num_workers': opt.workers, 'shuffle': True})
        val_vis_loader = data.frame_provider({'vis_feat': vis_feat_files['val'], 'vis_ids': val_vis_ids, 'pin_memory': True,
                                            'batch_size': opt.batch_size, 'num_workers': opt.workers})
    else:
        train_loader = data.pair_provider({'vis_feat': vis_feat_files['train'], 'capfile': cap_file_paths['train'], 'pin_memory': True,
                                           'batch_size': opt.batch_size, 'num_workers': opt.workers, 'shuffle': True,
                                           'caption_mask': caption_mask})

        val_vis_loader = data.vis_provider({'vis_feat': vis_feat_files['val'], 'vis_ids': val_vis_ids, 'pin_memory': True,
                                            'batch_size': opt.batch_size, 'num_workers': opt.workers})
    if valCollection == 'iacc.3' or valCollection == 'gcc11val':
        cap_file_paths['val'] = os.path.join(rootpath, valCollection, 'TextData', val_set)
        val_txt_loader = data.txt_provider({'capfile': cap_file_paths['val'], 'pin_memory': True,
                                             'batch_size': opt.batch_size, 'num_workers': opt.workers})
    else:
        cap_file_paths['val'] = os.path.join(rootpath, valCollection, 'TextData', val_set, '%s.caption.txt'%valCollection)
        val_txt_loader = data.txt_provider({'capfile': cap_file_paths['val'], 'pin_memory': True,
                                            'batch_size': opt.batch_size, 'num_workers': opt.workers})
    ...
    if opt.evaluate:
        ...
        print("=> loaded checkpoint '{}' (epoch {}, best_perf {})"
             .format(resume_file, checkpoint['epoch'], checkpoint['best_perf']))
        if valCollection == 'iacc.3':
            cur_perf = validate_v3(model, val_txt_loader, val_vis_loader, checkpoint['epoch'], model_path)
        else:
            cur_perf = validate_v2(model, val_txt_loader, val_vis_loader, checkpoint['epoch'], metric=opt.metric)
        exit(0)
    ...
    if opt.resume:
        ...
        model.load_state_dict(checkpoint['model'])
        print("=> loaded checkpoint '{}' (epoch {}, best_perf {})"
             .format(opt.resume, checkpoint['epoch'], checkpoint['best_perf']))
        best_perf = checkpoint['best_perf']
        save_checkpoint({'epoch': 0, 'model': model.state_dict(), 'best_perf': best_perf,
                         'config': config, 'opt': opt}, is_best=True, logdir=model_path, only_best=True,
    ...
    for epoch in range(opt.num_epochs):
        ...
        if opt.save_negative:
            with open(os.path.join(model_path, 'max_negative_%d.txt'%epoch), 'w') as fw:
        ...
        #cur_perf = validate(model, data_loaders['val'], epoch, metric=opt.metric)
        if valCollection == 'iacc.3':
            cur_perf = validate_v3(model, val_txt_loader, val_vis_loader, epoch, model_path)
        else:
            cur_perf = validate_v2(model, val_txt_loader, val_vis_loader, epoch, metric=opt.metric)
...
def train(model, train_loader, fw=None):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        progbar.add(len(vis_input), values=[('data_time', data_time.val), ('batch_time', batch_time.val), ('loss', loss)])
...
def validate_v2(model, txt_loader, vis_loader, epoch, metric='mir'):
    ...
    (r1i, r5i, r10i, medri, meanri, miri, mAPi, negRanki) = evaluation.eval(label_matrix)
...
def validate_v3(model, txt_loader, vis_loader, epoch, model_path, metric='infAP'):
    ...
    for k, topic_set in enumerate(topic_sets):
        topic_ids = [line.strip().split()[0] for line in 
                open(os.path.join(opt.rootpath, opt.valCollection, 'TextData', '%s.avs.txt'%topic_sets[k]))]
        with open(pred_treceval_file, 'w') as fout:
            for index in range(inds.shape[0])[k*TOPIC_SIZE: (k+1)*TOPIC_SIZE]:
                assert (txt_ids[index] in topic_ids), '%s not in %s'%(txt_ids[index], topic_set)
                ind = inds[index][::-1][:MAX_RETURN]

                newlines = ['%s 0 %s %d %d %s' % ('1'+txt_ids[index], vis_ids[idx], i+1, MAX_SCORE-i, TEAM) for i, idx in enumerate(ind)]
                fout.write('\n'.join(newlines)+'\n')

        gt_file = os.path.join(opt.rootpath, opt.valCollection, 'TextData', 'avs.qrels.%s' % topic_sets[k])
        cmd = 'perl tv-avs-eval/sample_eval.pl -q %s %s' % (gt_file, pred_treceval_file)
        res = os.popen(cmd).read().split('\n')
        infAPs[k] = [float(line.split()[-1]) for line in res if 'infAP' in line and 'all' in line][0]
